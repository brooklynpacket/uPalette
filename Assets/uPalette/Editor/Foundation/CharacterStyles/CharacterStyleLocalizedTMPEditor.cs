...
{
    class Wrappo : ScriptableObject
    {
        public LocalizedAsset<TMP_FontAsset> fontAsset;
        public LocalizedAsset<Material> materialAsset;
    }
    
    public sealed class CharacterStyleLocalizedTMPEditor : EditorWindow
    {
        private const float WindowWidth = 300.0f;

        private CharacterStyleLocalizedTMP _characterStyle;
        ...
        private void OnGUI()
        {
            ...
            SerializedProperty serializedFontAsset = serializedObject.FindProperty("fontAsset");
            SerializedProperty serializedMaterialAsset = serializedObject.FindProperty("materialAsset");

            EditorGUILayout.PropertyField(serializedFontAsset, true);
            EditorGUILayout.PropertyField(serializedMaterialAsset, true);
            ...
            {
                _characterStyle.fontAsset = obj.fontAsset;
                _characterStyle.fontMaterial = obj.materialAsset;
        ...
        public event Action<CharacterStyleLocalizedTMP> OnValueChanged;

        private void Setup(CharacterStyleLocalizedTMP characterStyle)
        {
            _characterStyle = characterStyle;
            obj = ScriptableObject.CreateInstance<Wrappo>();
            obj.fontAsset = _characterStyle.fontAsset;
            obj.materialAsset = _characterStyle.fontMaterial;
            titleContent = new GUIContent(ObjectNames.NicifyVariableName(nameof(CharacterStyleLocalizedTMP)));
        ...
        public static CharacterStyleLocalizedTMPEditor Open(CharacterStyleLocalizedTMP characterStyle)
        {
            var window = CreateInstance<CharacterStyleLocalizedTMPEditor>();
